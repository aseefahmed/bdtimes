            ...
            <div class="header-section" id="user-headerbox">
                <div class="user-header-wrap">
                    <div class="user-photo">
						@if(Auth::user()->image == "")
							<img src="{{ asset('public/images/no_image.png') }}" alt="{{ Auth::user()->first_name }} {{ Auth::user()->last_name }}" />
						@else 
							<img src="{{asset('public/images/profiles/'.Auth::user()->image)}}" alt="{{ Auth::user()->first_name }} {{ Auth::user()->last_name }}" />
						@endif
                    </div>
                    <div class="user-info">
                        @if(Auth::user()->role == 9)
                        ...
                        @elseif(Auth::user()->role == 4)
                            <span class="user-profile">Visitor</span>
                        @endif
                    </div>
                ...
                <div class="user-options dropdown-box">
                    ...
                    <div class="drop-content basic">
                        <ul>
                            @if(Auth::user()->role == 4)
                                <li> <a href="{{ url('view/profile') }}"><i class="fa fa-user" aria-hidden="true"></i> Profile</a></li>
                            @else
                                <li> <a href="{{ url('view/profile/') }}"><i class="fa fa-user" aria-hidden="true"></i> Profile</a></li>
                            @endif
                        ...
                        <ul class="nav" id="main-nav">
                            ...
                            @if(Auth::user()->role == 1 )
                                ...
                                <li class=" has-child-item close-item">
                                    ...
                                    <ul class="nav child-nav level-1">
                                        <li><a href="{{ url('categories/menu') }}">Menus List</a></li>
                                    </ul>
                                </li>
                            ...
                            <li class=" has-child-item close-item">
                                ...
                                <ul class="nav child-nav level-1">
                                    ...
                                    <li><a href="{{ url('news/addvideo') }}">Add Video</a></li>
                                    <li><a href="{{ url('news/addpicture') }}">Add Picture</a></li>
                            ...
                            @if(Auth::user()->role ==1)
								...
								<li class=" has-child-item close-item">
									...
									<ul class="nav child-nav level-1">
										<li><a href="{{ url('media/list') }}">Media List</a></li>
									</ul>
								</li>
								...
								<li class=" has-child-item close-item">
									...
									<ul class="nav child-nav level-1">
										<li><a href="{{ url('polls/add') }}">Create Poll</a></li>
										<li><a href="{{ url('polls/list') }}">Polls List</a></li>
									</ul>
								</li>
            ...
            <div class="content-header">
                <div class="leftside-content-header col-md-6">
                    <ul class="breadcrumbs">
                        ...
                        @else
                            <li><i class="fa fa-home" aria-hidden="true"></i><a href="#">{{ucwords(strtolower(Route::current()->getName()))}}</a></li>
                        @endif
                    </ul>
                </div>
...
/*
    ...
    $('.savePostBtn').click(function(){
        ...
        /* if($('#news_details').val()=="" || $('#featured_image').val()=="" || $('#news_title').val()=="" || $('#main_category').val()==0){
            $('#warning_modal').modal('toggle');
            return false;
        }*/

        if($('#news_details').val()==""  || $('#news_title').val()=="" || $('#main_category').val()==0){
            $('#warning_modal').modal('toggle');
            return false;
        }
    ...
    $('.saveVideoPostBtn').click(function(){
        //alert('test');
        ...
        if($('#news_title').val()==""  || $('#featured_image').val()=="" || $('#main_category').val()==0 || $('#video_link').val()==""){
            $('#warning_modal').modal('toggle');
            return false;
        }

        $('.saveVideoPostBtn').attr('disabled', 'disabled');
        $('#ajax_loading').css('display', 'inline');

        console.log($('#news_title'));
        formData = new FormData($("#addNewsForm")[0]);
        //console.log(formData);
        alert($('#news_title').val());
        $.ajax({
            url: "{{ url('news/addvideo') }}",
            method: "POST",
            data: formData,
            contentType: false,
            cache: false,
            ...
            success: function(result){
                ...
                $('#ajax_loading').css('display', 'none');
//                toastr.warning('Video News has been created successfully!', 'Notification')
//                toastr.options.closeButton = true;
                window.location.href = app.host + 'news/list';
            }
        }).fail(function(result){
    ...
    $('.savePollsBtn').click(function(){
		$('.savePollsBtn').attr('disabled', 'disabled');
		$('.savePollsBtn').css('display', 'inline');
		
		var flag = $(this).attr('flag');
		$('#flag').val(flag);
		formData = new FormData($("#addPollForm")[0]);
		console.log(formData)
        $.ajax({
            url: "{{ url('polls/add') }}",
            method: "POST",
            ...
            success: function(result){
                ...
                toastr.warning('News has been created successfully!', 'Notification')
                toastr.options.closeButton = true;
                window.location.href = app.host + 'polls/list';
            }
        }).fail(function(result){
            console.log(result)
        });
	...
	$('.uploadImage').click(function(){
        ...
        $.ajax({
            ...
            success: function(result){
				...
				$('#ajax_loading').css('display', 'none');
                toastr.warning('Images has been created successfully!', 'Notification')
                toastr.options.closeButton = true;
                window.location.href = app.host + 'media/list';
            }
        }).fail(function(result){
            console.log(result)
        });
    ...
    $('#main_category').change(function(){
        parent_category = $('#main_category').val();
        //alert(parent_category);

        $.ajax({
            url: "{{ url('category/searchSubCategory') }}/"+parent_category,
            method: "GET",
            contentType: false,
            cache: false,
            processData:false,
    ...
    $('#division').change(function(){
        parent_division = $('#divisions').val();

        $.ajax({
            url: "{{ url('division/searchdistrict') }}/"+parent_division,
            method: "GET",
            contentType: false,
            cache: false,
            processData:false,
    ...
    $('#district').change(function(){
        ...
        $.ajax({
            url: "{{ url('division/district/searchupazila') }}/"+parent_district,
            method: "GET",
            contentType: false,
            cache: false,
            processData:false,
	...
	$('#updateNewsBtn').click(function(){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                $('#ajax_loading').css('display', 'inline');
                toastr.warning('News has been updated successfully!', 'Notification')
                toastr.options.closeButton = true;
                window.location.href = app.host + 'news/list';
            }
        }).fail(function(result){
	...
	$('#updatePollBtn').click(function(){
		...
		$('#updatePollBtn').attr('disabled', 'disabled');
		$('#ajax_loading').css('display', 'inline');
		var poll_id = $('#updatePollBtn').attr('poll_id');
		formData = new FormData($("#editPollForm")[0]);
		console.log(formData)
        $.ajax({
            url: "{{ url('polls/edit') }}/"+poll_id,
            method: "POST",
            data: formData,
            contentType: false,
            cache: false,
	...
	$('#updateCatBtn').click(function(){
		...
		$('#updateCatBtn').attr('disabled', 'disabled');
		$('#ajax_loading').css('display', 'inline');
		var cat_id = $('#updateCatBtn').attr('cat_id');
		formData = new FormData($("#editCatForm")[0]);
		console.log(formData)
        $.ajax({
            url: "{{ url('cat/edit') }}/"+cat_id,
            method: "POST",
            data: formData,
            contentType: false,
            cache: false,
            ...
            success: function(result){
				...
				$('#ajax_loading').css('display', 'inline');
                toastr.warning('Category has been updated successfully!', 'Notification')
                toastr.options.closeButton = true;
                window.location.href = app.host + 'categories/list';
            }
        }).fail(function(result){
    ...
    $('#updateStatusBtn').click(function(){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                $('#ajax_loading').css('display', 'inline');
                toastr.warning('Status has been updated successfully!', 'Notification')
                toastr.options.closeButton = true;
                window.location.href = app.host + 'categories/menu';
            }
        }).fail(function(result){
            console.log(result)
        });
	...
	$('#updateAssetBtn').click(function(){
		...
		$('#updateAssetBtn').attr('disabled', 'disabled');
		$('#ajax_loading').css('display', 'inline');
		var asset_id = $('#updateAssetBtn').attr('asset_id');
		formData = new FormData($("#editAssetForm")[0]);
		console.log(formData)
        $.ajax({
            url: "{{ url('media/edit') }}/"+asset_id,
            method: "POST",
            data: formData,
            contentType: false,
            cache: false,
	...
	$('#search_news_btn').click(function(){
		formData = new FormData($("#myForm")[0]);
		$('#table_div').html("<div class='col-md-12 text-center'><img class='text-center' src='"+"{{asset('public/images/Ellipsis.gif')}}"+"'></div>");
		$.ajax({
            url: "{{ url('news/search') }}",
            method: "POST",
            data: formData,
    ...
    $('#edit_user').click(function(){
        ...
        $.ajax({
            ...
            success: function(result){
                ...
                // window.location.href = "{{url('view/profile')}}";
            }
        }).fail(function(result){
