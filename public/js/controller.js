var app = {};
//app.host = "http://localhost/project_bdtimes/";
    ...
    $scope.doLogin = function(){
        ...
        $http.post('processLogin', data, config).success(function (result, status) {
            ...
            else
            {
                ...
                $window.location.href = 'dashboar11d';
            }
...
angular.module('myApp').controller('NewsController', function($scope, $http, $window){
	...
	$scope.confirmRemovePoll = function(){
		$http.get(app.host + 'polls/remove/'+$scope.poll_id).then(function(response){
            toastr.success('Poll has been removed successfully.', 'Notification')
            toastr.options.closeButton = true;
	...
	$scope.confirmRemoveCat = function(){
		$http.get(app.host + 'cat/remove/'+$scope.cat_id).then(function(response){
            toastr.success('Categories has been removed successfully.', 'Notification')
            toastr.options.closeButton = true;
...
angular.module('myApp').controller('MediaController', function($scope, $http, $window){
    $scope.deleteMedia = function(id){
		$scope.asset_id = id;
		$('#modalSlideUp2').modal('show')
	}
	
	$scope.confirmRemoveMedia = function(){
		$http.get(app.host + 'media/remove/'+$scope.asset_id).then(function(response){
            toastr.success('Media has been removed successfully.', 'Notification')
            toastr.options.closeButton = true;
