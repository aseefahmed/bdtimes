...
<!-- double ad -->
    ...
    <div class="single-page-news-area section-padding padding-top-zero">
        <div class="container">
            <div class="row">
                <div class="col-md-8">
                        ...
                        <div class="single-page-news">
                            ...
                            <div class="report-info">
                            	@if($news_details[0]->image_caption != "")
                                	<p>{{$news_details[0]->image_caption}} <span> | Bangladesh Times</span></p>
                                @endif
                            </div>
                            <!-- sub headding dispaly none kora ache. sub heading dewa hole inline css style="display:block" add hobe-->
                            @if($news_details[0]->sub_title !="")
                            	<h4 class="sub-hedding">
									{{$news_details[0]->sub_title}}
	                            </h4>
                            @endif
                            <h3>
                                    ...
                                    <div class="col-xs-6">
                                        <div class="reporter-info">
                                            ...
                                            <div class="profile-info">
												...
												?>
                                                <p>{{ date_format(date_create($news_details[0]->created_at), 'H:i l, F j, Y') }}</p>
                                            </div>
                                        </div>
                                    </div>
...
'', 'menubar=no,toolbar=no,resizable=yes,scrollbars=yes,height=600,width=600');return false;"><i class="fa fa-google-plus"></i></a></li>
                            ...
                            <div class="col-md-6">
								<div class="comment-section">
									...
									{{ csrf_field() }}
										...
										<input class="btn btn-success pull-right" id="submit_comment_btn" type="submit" value="Send">
									</form>
								</div>
							</div>
									...
									@foreach($comments as $comment)
										<div class="single-demo-comment">
											...
											<div class="comment-meta">
												...
												<span>{{ date_format(date_create($comment->created_at), 'M d, Y') }} at {{ date_format(date_create($comment->created_at), 'h:i a') }}</span>
												<p>{{ $comment->details }}</p>
												@if(count(getReplies($comment->id))>0)
													@foreach(getReplies($comment->id) as $my_comment)
														<div class="single-demo-comment">
															<div class="comment-dp">
																@if($my_comment->user_id == 0)
																	<img src="{{url('public/assets/img/single-news-item-imgs/profile.png')}}" alt="">
																@else
																	<img src="{{url('public/images/profiles/'.$my_comment->image)}}" alt="">
																@endif
															</div>
															<div class="comment-meta">
																<h3>
																	@if($my_comment->user_id == 0)
																		Anonymous User
																	@else
																		{{ $my_comment->first_name}} {{ $my_comment->last_name}}
																	@endif
																</h3>
																<span>{{ date_format(date_create($my_comment->created_at), 'M d, Y') }} at {{ date_format(date_create($my_comment->created_at), 'h:i a') }}</span>
																<p>{{ $my_comment->details }}</p>
												...
												@endif
												<div class="reply">
													<a href="#" comment_id="{{$comment->id}}" news_id="{{$news_details[0]->id}}" class="comment-reply-link">Reply</a>
												</div>
											</div>
										</div>
									@endforeach
						...
						<div class="col-xs-12">
                            <div class="row">
                                <div class="more-news-singlepage">
                                    <h3 class="section-heading"><a href="">More News</a></h3>
											...
											@if($key<10)
												<div class="single-thumbnail-item-2">
													...
													<div class="news-content">
														...
														<a href="{{ url('news/front/details/'.$news->id) }}" class="news-readmore-btn">Read More</a>
													</div>
												</div>
											@endif
										@endforeach
                ...
                <div class="col-md-4">
                    <div class="right-content-section single-page">
                        ...
                        <!--most news section-->
                        <div class="single-page-top-news news-section-im70 margin-top-10">
                            <h3 class="section-heading"><a href="">Most Commented</a></h3>
							@foreach($popular_news as $key=>$news)
								@if($key> 9  && $key<15)
									<div class="single-thumbnail-item-2">
										<div class="thumbnail-bg"  style="background-image: url({{ url('public/images/news/featured/'.$news->featured_image)}})"></div>
